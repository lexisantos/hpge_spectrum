...
iso = '137Cs'
fuente = 'Cs137_76072-440'  #'Co60_76057-440'
...
path_incog = "D:/Proyecto LINT at Prompt Gamma/2025-06-09_fluxatPGwithfoils/11.06.2025_PG_LINT/-txt/DF_Cs137_76072_sep3_rON_11.06.2025.txt"
spec_incog = act.fromspec(path_incog, coef_en = spec_cal.coef_en[::-1])
pico_incog = spec_incog.ROI(list(ROIs.values())[0], n_bkg=n_back)

E_incog = pico_incog['en_max']
cps_pico = np.array([pico_incog['net'], pico_incog['net_err']])/spec_incog.tlive - cps_ROIs[iso]

Act_incog, Act_RA3, diff = act.Actividad(np.array([E_incog]), np.array([cps_pico[0]]), np.array([cps_pico[1]]), 
                          spec_incog.treal, coef_tabla, isfromRA3=True, Fuente=fuente, dt = fecha_incog, var_mu = var_mus)
...
t_inicio = datetime(2025, 7, 29, 10, 35, 12)
...
for alambre in nromed:
    ...
    for ii, med in enumerate(medno):
        data = act.fromspec(path_job(alambre, med), spec_cal.coef_en)
