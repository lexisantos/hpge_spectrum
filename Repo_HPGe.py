...
class fromspec:
    def __init__(self, path, coef_en=[]):
        ...
        df_data = pd.read_table(path, names=['col', 'value'], index_col='col', skipfooter=fcut, engine = 'python', delimiter=':  |\n')
...
class NAA_calib:
    ...
    def cal_eff(self, ROIs, spec_cal, grado_pol: int = 1, n_bkg: int = 3, criterio: float=0, tolerancia: float=0.0025):
        ...
        eff = cps_net/(self.act_cal[0]*data_sel[:, 0])
        try:
            eff_err = eff*np.sqrt((cps_net_err/cps_net)**2 + (data_sel[:, 1]/data_sel[:, 0])**2 
                                  + (self.act_cal[1]/self.act_cal[0])**2)
        except:
            eff_err = None
        coef, perr, chi2, res, pvalor, ddof, rhos, var_mu = ajuste_pol(grado_pol, np.log(Epeak_spec), np.log(eff), y_err=eff_err/eff)
